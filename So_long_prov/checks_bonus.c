...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/02 16:43:10 by jverdier          #+#    #+#             */
/*   Updated: 2024/07/02 17:12:51 by jverdier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_strange(char *map_str)
{
	...
	while (map_str[i])
	{
		if (map_str[i] != CEXIT && map_str[i] != CFLOOR && map_str[i] != CWALL \
			&& map_str[i] != CITEM && map_str[i] != CPLAYER \
			&& map_str[i] != CENNEMY && map_str[i] != '\n' \
			&& map_str[i] != '\0')
		{
